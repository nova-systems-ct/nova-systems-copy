...
import { handleSmsInbound, handleCallStatus, handleFormIntake } from './_wave1Handlers.js';
...
export async function handleWave1(req, res, op, deps) {
  ...
  if (op === 'sms-inbound' || op === 'call-status') {
    if (req.method !== 'POST') return res.status(405).json({ error: 'Method not allowed' });
    const url = base ? `${base}/api/client?resource=wave1&op=${op}` : '';
    const params = req.body && typeof req.body === 'object' ? req.body : {};
    const fn = op === 'sms-inbound' ? handleSmsInbound : handleCallStatus;
  ...
  if (op === 'settings') {
    ...
    if (b.rotate_form_token === true) rec.form_token = `${Date.now().toString(36)}${Math.random().toString(36).slice(2, 12)}`;
